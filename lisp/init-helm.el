  ...
  (progn
    (bind-key
     ...
     (defhydra hydra-helm () "helm"
       ...
       ("x" whit-run-external-command "run" :exit 1)
       ("u" helm-unicode "unicode")))

    (use-package helm-external
      :init
      (progn
        (defun whit-run-external-command (arg)
          (interactive "P")
          (when arg
...
(use-package helm-swoop
  ...
  :config
  (progn
    (defun whit-helm-pre-input ()
      "")
    (setq helm-swoop-pre-input-function 'whit-helm-pre-input)))
