    ...
    (with-current-buffer target-buffer
      (mapcar (lambda (job)
		(let* ((name (cdr (assoc 'name job)))
		  ...
		  (cond
		   ((string= color  "red")
		    (insert (propertize "Ï" 'face `(:foreground ,color))))
		   ((string= color "yellow")
		    (insert (propertize "Ï" 'face `(:foreground ,color))))
		   ((string= color  "blue")
		    (insert (propertize "Ï" 'face `(:foreground ,color))))
		   ((string= color  "grey")
		    ...
		    (insert (propertize "Ï" 'face `(:foreground "grey"))))
		   ((string= color "disabled")
		    (insert (propertize "Ï" 'face `(:foreground "black"))))
		   ((string= (subseq color -6) "_anime")
		    (insert (propertize "Ï" 'face `(:foreground ,(subseq color 0 -6)))))
